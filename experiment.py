...
def train(d_model, latent_dim, layers, nqubits, training_samples, circuit, n_epochs, samples, lr, lr_d, avg, dataset, folder):
    ...
    initial_params = tf.Variable(np.random.uniform(-0.15, 0.15, 66))
    ...
    for i in range(n_epochs):
        ...
        g_loss.append(loss)
        np.savetxt(f"{folder}/PARAMS_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}_{lr_d}_{avg}_cern", [initial_params.numpy()], newline='')
        np.savetxt(f"{folder}/dloss_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}_{lr_d}_{avg}_cern", [d_loss], newline='')
        np.savetxt(f"{folder}/gloss_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}_{lr_d}_{avg}_cern", [g_loss], newline='')
        if i % 500 == 0:
            with open(f"{folder}/ALL_PARAMS_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}_{lr_d}_{avg}_cern", "ab") as f:
...
def build_and_train_model(lr_d=1e-1, lr=5e-2, n_epochs=10, batch_samples=32, latent_dim=3, layers=1,
                          training_samples=1000, pixels=64, nqubits=6, avg=1, dataset=None, folder=None):
    ...
    circuit.add(gates.CZ(7,6))
    for q in [1,3,4,6]:
        circuit.add(gates.RY(q,0))
    circuit.add(gates.CZ(1,3))
    circuit.add(gates.CZ(8,6))
    for q in [3,6]:
        circuit.add(gates.RY(q, 0))
    circuit.add(gates.CZ(3,4))
    circuit.add(gates.RY(4,0))
    circuit.add(gates.CZ(4,6))
    circuit.add(gates.RY(6,0))
    circuit.add(gates.CZ(6,4))
    circuit.add(gates.RY(6,0))
    circuit.add(gates.RY(4,0))
    circuit.add(gates.CZ(4,3))
    circuit.add(gates.RY(4,0))
    circuit.add(gates.RY(3,0))
    circuit.add(gates.CZ(6,8))
    circuit.add(gates.RY(6,0))
    circuit.add(gates.RY(8,0))
    circuit.add(gates.CZ(3,1))
    circuit.add(gates.RY(3,0))
    circuit.add(gates.RY(1,0))
    for q in [(1,0), (3, 2), (4, 5), (6, 7)]:
        circuit.add(gates.CZ(*q))
        circuit.add(gates.RY(q[0], 0))
        circuit.add(gates.RY(q[1], 0))
