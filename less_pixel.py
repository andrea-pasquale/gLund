...
def train(d_model, latent_dim, layers, nqubits, training_samples, discriminator, circuit, n_epochs, samples, lr, pixels, lr_d):
    ...
    for i in range(n_epochs):
        ...
        g_loss.append(loss)
        np.savetxt(f"less_image_test/PARAMS_4pxls_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}_{lr_d}", [initial_params.numpy()], newline='')
        np.savetxt(f"less_image_test/dloss_4pxls_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}_{lr_d}", [d_loss], newline='')
        np.savetxt(f"less_image_test/gloss_4pxls_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}_{lr_d}", [g_loss], newline='')
